...
def game(isMouse=False):
    ...
    sub_font = pygame.font.Font(Font.neue_font, 35)
    sub_small_font = pygame.font.Font(Font.neue_font, 30)
    pop_up_font = pygame.font.Font(Font.edit_undo_font, 53)
    ...
    def redraw_window():
        ...
        # Draw Text
        Assets.text.draw(f'{player.get_level()} / 12', sub_small_font, Colors.CYAN,
        ...
        if player.get_level() >= 12 and boss_entry:
    ...
    while player.run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length if player.get_level() < 12 else 1):
                enemies.append(Enemy(
                    random.randrange(50, config.WIDTH - 100),
                    random.randrange(-1200, -100),
                    random.choice(['easy', 'medium', 'hard']) if player.get_level() < 12 else 'boss')
                )
